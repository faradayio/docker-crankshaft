...
def create_and_predict_segment_agg(target, features, target_features,
    ...
    straight form the SQL calling the function.
        ...
        Input:
            ...
            @param target_ids: A 1D array of target_ids that will be used to
                               associate the results of the prediction with the
                               rows which they come from
    ...
    return zip(target_ids, prediction, np.full(prediction.shape,
                                               accuracy_array))
    ...
    """
        ...
        Input:
            ...
            @param model_params: A dictionary of model parameters, the full
                                 specification can be found on the scikit learn
                                 page for GradientBoostingRegressor:
...
def predict_segment(model, features, target_query):
    ...
    Use the provided model to predict the values for the new feature set
        Input:
            ...
            @features: A list of features to use in the model prediction (list
                       of column names)
            @target_query: The query to run to obtain the data to predict on
                           and the cartdb_ids associated with it.
    """
    ...
    try:
        cursor = plpy.cursor('''
          SELECT Array[{joined_features}] As features
            ...
            '''.format(joined_features=joined_features,
    ...
    while True:
        ...
        # Need to fix this. Should be global mean.
        #  This will cause weird effects
