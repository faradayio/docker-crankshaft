...
class SegmentationTest(unittest.TestCase):
    ...
    def test_create_and_predict_segment(self):
        ...
        plpy._define_result('select \* from  \(select \* from training\) a '
                            'limit 1', rows)
        plpy._define_result('.*from \(select \* from training\) as a',
                            training_data)
        plpy._define_result('select array_agg\(cartodb\_id order by '
                            'cartodb\_id\) as cartodb_ids from \(.*\) a', ids)
        ...
        accuracy = np.sqrt(np.mean(
          np.square(np.array(prediction) - np.array(test_y))))
